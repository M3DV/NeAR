...
from near.datasets.seg_dataset import DistortedSegDataset
...
def main():
    ...
    # Datasets
    train_set = DistortedSegDataset(root=cfg['data_path'], resolution=cfg["resolution"])
    ...
    logs = ['loss', 'iou', 'dice', 'dice_to_clean', 'iou0', 'iou1', 'dice0', 'dice1', 'dice_global']
...
def test_epoch(model, loader, save_dir, cfg, print_freq=1, is_test=True):
    ...
    with torch.no_grad():
        for batch_idx, (x, clean_seg, y) in enumerate(loader):
            x = to_var(x)
            y = to_var(y)
            clean_seg = to_var(clean_seg)

            pred_logit = model(x)
            y_one_hot = categorical_to_one_hot(y, dim=1, expand_dim=False)
            clean_one_hot = categorical_to_one_hot(clean_seg, dim=1, expand_dim=False)

            pred_classes = pred_logit.argmax(1)

            name = info.iloc[idx]['id']
            idx += 1
            ...
            dice_to_clean = cal_batch_dice(pred_logit, clean_one_hot)

            logs = [loss.item(), iou[1:].mean(), dice[1:].mean(), dice_to_clean[1:].mean()]+ \
            ...
            if batch_idx % print_freq == 0:
                res = '\t'.join([
                    ...
                    'DICE_TO_CLEAN %.4f (%.4f)' % (meters.val[3], meters.avg[3]),
                ])
                print(res)
